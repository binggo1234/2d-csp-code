...
def _repack_fixed_parts_ccf(
    ...
    *,
    ...
    # When SA is enabled, too many random starts becomes prohibitively expensive.
    ...
    for t in range(starts):
        ...
        # Simulated annealing on order; each candidate order is evaluated by
        # re-packing with the same BLF+CCF placer.
        cur_board = b0
        cur_info = info0
        ...
        for _ in range(iters):
            ...
            if delta >= 0.0:
                ...
                # score is in mm-units; T is also in mm-units
