...
def _merge_collinear_edges(tr: List[Point], *, eps: float = 1e-9, col_eps: float = 1e-6) -> List[Point]:
    ...
    for p in tr[1:]:
        ...
        while len(out) >= 3:
            ...
            if not _is_collinear(a, b, c, eps=col_eps):
                break
            # Keep 180-degree reversals (A->B->A) and backtracking turns.
            # Only merge when consecutive collinear edges continue forward.
...
def compute_toolpath_trails(
    ...
    *,
    ...
    if any(bool(getattr(s, "shared", False)) for s in segs2):
        ...
        # For non-shared routing we must preserve multi-edges exactly.
        # Otherwise touching part boundaries can be collapsed away, which
        # underestimates cut length for the no-SEC baselines.
        simp_g = base_g
