...
def _pick(row: Dict[str, Any], *keys: str, default=0.0):
    for key in keys:
        if key in row:
            return row[key]
    return default
...
def build_summary_stats(totals_all: List[Dict[str, Any]], variants: List[str]) -> List[Dict[str, Any]]:
    ...
    for v in variants:
        ...
        rows.append(dict(
            ...
            U_excl_last_mean=_mean([r.get("U_avg_excl_last", r["U_avg"]) for r in data]),
            U_excl_last_std=_std([r.get("U_avg_excl_last", r["U_avg"]) for r in data]),
            ...
            N_tabs_mean=_mean([_pick(r, "n_tabs_sum", "N_tabs_sum", default=0.0) for r in data]),
            N_tabs_std=_std([_pick(r, "n_tabs_sum", "N_tabs_sum", default=0.0) for r in data]),
            Risk_slender_mean=_mean([_pick(r, "Risk_slender_avg", "Risk_slender_mean", default=0.0) for r in data]),
            Risk_slender_std=_std([_pick(r, "Risk_slender_avg", "Risk_slender_mean", default=0.0) for r in data]),
            T_cut_mean=_mean([r.get("T_cut_sum", 0.0) for r in data]),
            T_cut_std=_std([r.get("T_cut_sum", 0.0) for r in data]),
            T_air_mean=_mean([r.get("T_air_sum", 0.0) for r in data]),
            T_air_std=_std([r.get("T_air_sum", 0.0) for r in data]),
            T_lift_mean=_mean([r.get("T_lift_sum", 0.0) for r in data]),
            T_lift_std=_std([r.get("T_lift_sum", 0.0) for r in data]),
