...
def _write_rows_csv(path: Path, rows: List[Dict[str, Any]]) -> None:
    ...
    preferred = [
        ...
        "U_avg", "U_avg_excl_last", "U_mean",
        "L_shared_sum", "N_lift_sum", "L_air_sum",
        "L_cut_sum", "L_cut_base_sum", "L_dual_pass_extra_sum", "L_kerf_extra_sum", "L_lead_sum",
        "T_cut_sum", "T_air_sum", "T_lift_sum",
        "Risk_shared_sum", "Risk_slender_sum",
        "T_est_sum",
    ]
...
def _cfg_for_variant(cfg, variant: str):
    if variant != "ablation_routing":
        return cfg
    cfg_ablation = copy.deepcopy(cfg)
    cfg_ablation.ANTI_SHIFT_ENABLE = False
    cfg_ablation.TAB_ENABLE = False
    cfg_ablation.ROUTE_PRIORITY = "none"
    cfg_ablation.ROUTE_START_POLICY = "none"
    cfg_ablation.ROUTE_CCW = False
    cfg_ablation.ROUTE_HIERARCHICAL = False
    cfg_ablation.ROUTE_RAMP_ENABLE = False
    cfg_ablation.ROUTE_ENTRY_JUNCTION_PENALTY_MM = 0.0
    cfg_ablation.SHARED_MAX_CONTINUOUS_CUT = 999999.0
    cfg_ablation.SHARED_REQUIRE_DUAL_ON_EDGEBAND = False
    cfg_ablation.SHARED_PASS_MODE = "none"
    cfg_ablation.SHARED_PASS_MULT = 1.0
    return cfg_ablation


def _dump_effective_variant_configs(cfg, out_case_dir: Path, variants: List[str]) -> None:
    for variant in variants:
        cfg_eff = _cfg_for_variant(cfg, variant)
        dump_config(
            cfg_eff,
            out_case_dir,
            extra={
                "script": "main_stepF.py",
                "variant": variant,
                "effective_cfg": True,
            },
            filename=f"effective_cfg_{variant}.json",
        )
...
def run_one_variant(parts, cfg, seed: int, variant: str, out_case_dir: Path) -> Dict[str, Any]:
    ...
    if variant == "baselineA":
        boards = pack_baselineA(
            ...
            safe_gap=safe_gap,
            touch_tol=touch_tol,
            place_mode=getattr(cfg, "BASELINE_PLACE_MODE", "blf"),
        )
        share_mode = "none"
        ...
        boards = pack_baselineB(
            ...
            place_mode_override=getattr(cfg, "BASELINE_PLACE_MODE", ""),
        )
        share_mode = "none"
    elif variant in {"proposed_shared", "ablation_routing"}:
        boards = pack_proposed_shared(
            ...
            stage2_sa_patience=getattr(cfg, "INNER_REPACK_SA_PATIENCE", 200),
            stage2_sa_auto=getattr(cfg, "INNER_REPACK_SA_AUTO", False),
            stage2_sa_base_n=getattr(cfg, "INNER_REPACK_SA_BASE_N", 200),
            stage2_sa_min_iters=getattr(cfg, "INNER_REPACK_SA_MIN_ITERS", 10),
            stage1_method=getattr(cfg, "STAGE1_METHOD", "sa"),
            stage1_sa_iters=getattr(cfg, "STAGE1_SA_ITERS", 2000),
            stage1_place_mode=getattr(cfg, "STAGE1_PLACE_MODE", "blf"),
            stage1_lns_enable=getattr(cfg, "STAGE1_LNS_ENABLE", False),
            stage1_lns_prob=getattr(cfg, "STAGE1_LNS_PROB", 0.15),
            stage1_lns_destroy_frac=getattr(cfg, "STAGE1_LNS_DESTROY_FRAC", 0.08),
            # Stage1 ALNS knobs
            stage1_alns_iters=getattr(cfg, "STAGE1_ALNS_ITERS", 600),
            alns_w1=getattr(cfg, "ALNS_W1", 1.0),
            alns_w2=getattr(cfg, "ALNS_W2", 0.25),
            alns_w3=getattr(cfg, "ALNS_W3", 0.10),
            alns_destroy_frac=getattr(cfg, "ALNS_DESTROY_FRAC", 0.10),
            alns_destroy_min=getattr(cfg, "ALNS_DESTROY_MIN", 6),
            alns_destroy_max=getattr(cfg, "ALNS_DESTROY_MAX", 120),
            alns_segment_len=getattr(cfg, "ALNS_SEGMENT_LEN", 50),
            alns_reaction=getattr(cfg, "ALNS_REACTION", 0.2),
            alns_T0=getattr(cfg, "ALNS_T0", 0.05),
            alns_alpha=getattr(cfg, "ALNS_ALPHA", 0.995),
            # fitness eval params (align with metrics)
            feed_cut=getattr(cfg, "FEED_CUT", 12000.0),
            feed_air=getattr(cfg, "FEED_AIR", 30000.0),
            t_lift=getattr(cfg, "T_LIFT", 0.8),
            ...
            route_hierarchical=getattr(cfg, "ROUTE_HIERARCHICAL", False),
            route_large_frac=getattr(cfg, "ROUTE_LARGE_FRAC", 0.20),
            route_ramp_enable=getattr(cfg, "ROUTE_RAMP_ENABLE", False),
            route_ramp_len=getattr(cfg, "ROUTE_RAMP_LEN", 0.0),
            anti_shift_enable=getattr(cfg, "ANTI_SHIFT_ENABLE", False),
            anti_shift_area_m2=getattr(cfg, "ANTI_SHIFT_AREA_M2", 0.05),
            anti_shift_ar=getattr(cfg, "ANTI_SHIFT_AR", 5.0),
            anti_shift_two_pass=getattr(cfg, "ANTI_SHIFT_TWO_PASS", False),
        )
        share_mode = "union"
    else:
    ...
    for b in boards:
        row = compute_board_metrics(
            ...
            tab_len=getattr(cfg, "TAB_LEN", 0.0),
            tab_corner_clear=getattr(cfg, "TAB_CORNER_CLEAR", 0.0),
            tab_skip_trim_edge=getattr(cfg, "TAB_SKIP_TRIM_EDGE", False),
            tab_adaptive=getattr(cfg, "TAB_ADAPTIVE", False),
            tab_slender_ratio=getattr(cfg, "TAB_SLENDER_RATIO", 6.0),
            tab_slender_extra=getattr(cfg, "TAB_SLENDER_EXTRA", 0),
            tab_small_area_extra=getattr(cfg, "TAB_SMALL_AREA_EXTRA", 0),
            line_snap_eps=getattr(cfg, "LINE_SNAP_EPS", 0.0),
            min_shared_len=getattr(cfg, "MIN_SHARED_LEN", 0.0),
            shared_enable_edgeband=getattr(cfg, "SHARED_ENABLE_EDGEBAND", False),
            shared_min_len_edgeband=getattr(cfg, "SHARED_MIN_LEN_EDGEBAND", 0.0),
            shared_max_continuous_cut=getattr(cfg, "SHARED_MAX_CONTINUOUS_CUT", 0.0),
            shared_hold_bridge_len=getattr(cfg, "SHARED_HOLD_BRIDGE_LEN", 0.0),
            shared_require_dual_on_edgeband=getattr(cfg, "SHARED_REQUIRE_DUAL_ON_EDGEBAND", True),
            shared_pass_mode=getattr(cfg, "SHARED_PASS_MODE", "none"),
            shared_pass_mult=getattr(cfg, "SHARED_PASS_MULT", 1.0),
            shared_risk_w=getattr(cfg, "SHARED_RISK_W", 0.0),
            route_start_policy=getattr(cfg, "ROUTE_START_POLICY", "none"),
            route_priority=getattr(cfg, "ROUTE_PRIORITY", "none"),
            route_ccw=getattr(cfg, "ROUTE_CCW", False),
            route_local_window=getattr(cfg, "ROUTE_LOCAL_WINDOW", 10),
            route_local_backtrack=getattr(cfg, "ROUTE_LOCAL_BACKTRACK", 2),
            route_small_first_area_mm2=1e6 * float(getattr(cfg, "ROUTE_SMALL_AREA_FIRST_M2", 0.0)),
            route_entry_junction_penalty_mm=getattr(cfg, "ROUTE_ENTRY_JUNCTION_PENALTY_MM", 0.0),
            route_hierarchical=getattr(cfg, "ROUTE_HIERARCHICAL", False),
            route_large_frac=getattr(cfg, "ROUTE_LARGE_FRAC", 0.20),
            route_ramp_enable=getattr(cfg, "ROUTE_RAMP_ENABLE", False),
            route_ramp_len=getattr(cfg, "ROUTE_RAMP_LEN", 0.0),
...
def _run_one_seed_worker(cfg_dict: Dict[str, Any], seed: int, variants: List[str], out_case_dir: str) -> List[Dict[str, Any]]:
    ...
    for v in variants:
        rows.append(run_one_variant(parts, _cfg_for_variant(cfg, v), seed, v, out_case_path))
    return rows
...
def run_stepF(cfg) -> None:
    ...
    Output:
      totals_baselineA.csv / totals_baselineB.csv / totals_proposed_shared.csv / totals_ablation_routing.csv
      summary_stats.csv
    """
    ...
    variants = ["baselineA", "baselineB", "proposed_shared", "ablation_routing"]
    if getattr(cfg, "DUMP_CONFIG", True):
        _dump_effective_variant_configs(cfg, out_case_dir, variants)
        ...
        while n_done < max_seeds:
            ...
            for j in range(batch):
                ...
                for v in variants:
                    ...
                    row = run_one_variant(parts, _cfg_for_variant(cfg, v), seed, v, out_case_dir)
