...
Additional shop-floor routing guards (configured in `src/inrp/cfg.py`):

- `ROUTE_SMALL_AREA_FIRST_M2`: strict small-part-first threshold.
- `ROUTE_ENTRY_JUNCTION_PENALTY_MM`: avoid entering at unstable high-degree junctions.
- `SHARED_MAX_CONTINUOUS_CUT` + `SHARED_HOLD_BRIDGE_LEN`: split over-long shared seams with hold bridges.
- `TAB_SKIP_TRIM_EDGE`: do not place tabs on trim-boundary edges, reducing edge fragmentation.
...
- Optional edge attributes:
  `EB_L, EB_R, EB_B, EB_T` (per-edge edgeband flags), and/or
  `edge_class` (`edgeband`, `non_edgeband`, `backboard`)
