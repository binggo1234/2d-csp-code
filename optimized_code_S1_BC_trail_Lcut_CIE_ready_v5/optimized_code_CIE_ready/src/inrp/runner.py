...
def _write_rows_csv(path: Path, rows: List[Dict[str, Any]]) -> None:
    ...
    try:
        ...
        # On Windows, the target may be locked by Excel/WPS/preview.
        ts = int(time.time())
        alt = path.with_name(f"{path.stem}.unlocked_{ts}{path.suffix}")
        os.replace(tmp, alt)
        logger.warning("[WRITE] %s locked; wrote to %s instead. Close the file and rerun to overwrite.", path, alt)
...
def run_stepF(cfg) -> None:
    ...
    for i in range(cfg.N_SEEDS):
        ...
        for v in variants:
            ...
            # printing keys adaptively (because aggregate_totals fields may differ)
            ...
            msg += f" runtime={row['runtime_s']:.3f}s"
            logger.info(msg)
