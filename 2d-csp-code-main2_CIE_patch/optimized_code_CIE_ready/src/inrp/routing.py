...
def stroke_representatives(
    ...
    """Decompose components into strokes (continuous cuts without lifting)."""
    used_edges: Set[int] = set()
    reps: List[Point] = []

    def _pick_rep(trail: List[Point]) -> Point:
        if not trail: return (0.0, 0.0)
        # Pick the start point of the stroke
        return trail[0]
    ...
    for comp in comps:
        ...
        def follow(u, v, eid0):
            ...
            cur = v
            while True:
                if cur not in comp or deg.get(cur, 0) != 2: break
                nxt = None
        ...
        # 1. Start from odd nodes or junctions (deg != 2)
        for u in comp:
            if deg.get(u, 0) == 2: continue
            for v, _, eid in adj.get(u, []):
                if eid in used_edges: continue
                used_edges.add(eid)
                reps.append(_pick_rep(follow(u, v, eid)))

        # 2. Cleanup cycles (deg == 2)
        for u in comp:
            for v, _, eid in adj.get(u, []):
                if eid in used_edges: continue
                used_edges.add(eid)
