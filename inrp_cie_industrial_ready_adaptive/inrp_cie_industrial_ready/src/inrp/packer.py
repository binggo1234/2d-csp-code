...
def pack_baselineA(parts: List, W: float, H: float, allow_rot: bool, *, trim: float = 0.0,
                   safe_gap: float = 0.0, touch_tol: float = 1e-6) -> List[Board]:
    return pack_multi_board(parts, W, H, allow_rot, trim=trim, safe_gap=safe_gap, touch_tol=touch_tol, order="size", place_mode="blf")
...
def pack_sequence_sa(
    ...
    current_boards = pack_multi_board(current_seq, W, H, allow_rot, trim=trim, safe_gap=safe_gap, touch_tol=touch_tol, order="none", place_mode="blf")
    ...
    for _ in range(int(iters)):
        ...
        try:
            new_boards = pack_multi_board(new_seq, W, H, allow_rot, trim=trim, safe_gap=safe_gap, touch_tol=touch_tol, order="none", place_mode="blf")
        except RuntimeError:
            continue
...
def pack_proposed_shared(
    ...
    if (stage1_method or "").lower().strip() == "sa":
        boards_stage1 = pack_sequence_sa(parts, W, H, allow_rot, trim=trim, safe_gap=safe_gap, touch_tol=touch_tol, seed=seed, iters=stage1_sa_iters)
    else:
        boards_stage1 = pack_baselineB(parts, W, H, allow_rot, trim=trim, safe_gap=safe_gap, touch_tol=touch_tol, restarts=restarts, seed=seed)
