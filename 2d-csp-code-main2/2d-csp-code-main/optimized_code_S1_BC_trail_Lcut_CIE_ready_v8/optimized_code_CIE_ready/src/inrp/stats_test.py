...
def pack_one_board(parts: List, W: float, H: float, allow_rot: bool, bid: int,
    ...
    for p in parts:
        ...
        if not ok:
            raise RuntimeError(f"Part too big: uid={p.uid} raw={p.pid_raw}")
...
def pack_multi_board(parts: List, W: float, H: float, allow_rot: bool,
    ...
    while left:
        ...
        if len(board.placed) == 0:
            bad = left[0]
            raise RuntimeError(f"Cannot place part: uid={bad.uid}")
...
def _repack_fixed_parts_ccf(
    ...
    for t in range(starts):
        ...
        for _ in range(iters):
            ...
            if delta >= 0.0 or (rng.random() < math.exp(delta / max(1e-9, T))):
