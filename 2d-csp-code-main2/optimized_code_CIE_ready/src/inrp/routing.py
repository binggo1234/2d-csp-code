...
def _mwpm_extra_exact(odds: List[Point], dist_map: Dict[Point, Dict[Point, float]]) -> float:
    ...
    # Distance matrix between odd vertices
...
# ---------------- Air move: Greedy Endpoint (Better than Centroid) ----------------
...
def air_length_by_components(comps: List[Set[Point]], start: Point = (0.0, 0.0)) -> float:
    ...
    Instead of using static centroids, we dynamically choose the entry point
    of the next component that is closest to the current cutter position.
    This simulates a more realistic greedy toolpath planning.
    """
    if not comps:
        return 0.0
    ...
    while unvisited:
        ...
        current_pos = nodes[best_node_idx]
