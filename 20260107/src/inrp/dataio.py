...
def read_sample_parts(path: str, trim: float, gap: float, tool_d: float = 6.0) -> List[Part]:
    ...
    读取 sample_parts.csv：
      ...
      - 生成 Part 列表，并计算 inflate 尺寸（用于排样/校验）：
          inflate = gap

        说明：
        - TRIM 被定义为“板材四边修边余量”，应当作用于板材可用域，而不是零件膨胀。
        - gap 可用于体现最小安全间隙（如果你想把 TOOL_D 进入可行域，可把 gap 设置为 TOOL_D 或 TOOL_D+δ）。
    """
    ...
    try:
        ...
        for row in reader:
            ...
            # inflate：排样用（只包含 gap；TRIM 作为板边修边余量在 packer/validate 中处理）
            inflate = float(gap)
            w = float(L0) + inflate
            h = float(W0) + inflate
